...
class AchCommands(commands.Cog):
    def __init__(self, bot):
        ...
        self.logger = (
            self.bot.get_cog("Logger").logger if self.bot.get_cog("Logger") else None
        )
    ...
    async def revoke_achievement(self, user_id, achievement_id, guild):
        ...
        if self.logger:
            self.logger.info(
                f"achievement revoked from {user_id}: {achievement_id} (-{xp_loss} xp)"
            )
    ...
    async def achievements(self, ctx, *args):
        ...
        if args[0] in ["-g", "--grant"]:
            ...
            success = await self.grant_achievement(
                str(member.id), ach_id, ctx.guild, ctx.channel
            )
            await ctx.send(
                f"granted achievement {ach_id} to {member.mention}"
                if success
                else f"{member.mention} already has this achievement"
            )
            ...
            await ctx.send(
                f"revoked achievement {ach_id} from {member.mention}"
                if success
                else f"{member.mention} doesnt have this achievement"
            )
