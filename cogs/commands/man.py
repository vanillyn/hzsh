...
class Docs(commands.Cog):
    ...
    async def man(self, ctx, *, command: str):
        ...
        try:
            ...
            try:
                ...
                name_content = result[name_start:synopsis_start].replace('NAME', '').strip()
                name_content = '\n'.join([line.strip() for line in name_content.split('\n') if line.strip()])

                synopsis_content = result[synopsis_start:description_start].replace('SYNOPSIS', '').strip()
                synopsis_content = '\n'.join([line.strip() for line in synopsis_content.split('\n') if line.strip()])

                desc_section = result[description_start:].replace('DESCRIPTION', '', 1).strip()
    ...
    async def aw(self, ctx, *, page: str):
        ...
        try:
            async with self.session.get(f"https://wiki.archlinux.org/api.php?action=parse&page={quote(page_title)}&format=json&prop=text") as resp:
                if resp.status != 200:
                ...
                class ArchWikiView(discord.ui.LayoutView):
                    def __init__(self, url, text, snippet):
                        ...
                        container = discord.ui.Container(
                            ...
                            discord.ui.TextDisplay(content="-# From [ArchWiki](https://wiki.archlinux.org/)."),
                            discord.ui.Separator(visible=True, spacing=discord.SeparatorSpacing.large),
                            discord.ui.TextDisplay(content=snippet),
                            discord.ui.Separator(visible=True, spacing=discord.SeparatorSpacing.small),
                            discord.ui.TextDisplay(content=f"-# read the full article here: {url}")
                        )
    ...
    async def gw(self, ctx, *, page: str):
        ...
        try:
            async with self.session.get(f"https://wiki.gentoo.org/api.php?action=parse&page={quote(page_title)}&format=json&prop=text") as resp:
                if resp.status != 200:
                ...
                class GentooWikiView(discord.ui.LayoutView):
                    def __init__(self, url, text, snippet):
                        ...
                        container = discord.ui.Container(
                            discord.ui.TextDisplay(content=f"## {page_title}"),
                            discord.ui.TextDisplay(content="-# From [Gentoo Wiki](https://wiki.gentoo.org/)."),
                            discord.ui.Separator(visible=True, spacing=discord.SeparatorSpacing.large),
                            discord.ui.TextDisplay(content=snippet),
                            discord.ui.Separator(visible=True, spacing=discord.SeparatorSpacing.small),
                            discord.ui.TextDisplay(content=f"-# read the full article here: {url}")
